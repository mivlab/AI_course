...
model = Net64x48()
model.load_state_dict(torch.load('params_25.pth'))
...
while True:
    ...
    for i in range(boxes.size(0)):
        box = boxes[i, :]
        label = f" {probs[i]:.2f}"
        b = box.numpy().astype(np.int32)
        img = cv2.resize(orig_image[b[1]:b[3], b[0]: b[2], :], (48, 64))
        img_tensor = transforms.ToTensor()(img)
        img_tensor = img_tensor.unsqueeze(0)
        prediction = model(Variable(img_tensor))
        pred = torch.max(prediction, 1)[1].item()
        label = 'face' if pred == 0 else 'mask'
